...
export class WebSocketControllerFactory {
  ...
  constructor(
    //@inject.context() public contextx: Context,
    private ctx: Context,
    private controllerClass: Constructor<{ [method: string]: Function }>
  ) {
    // this.ctx.bind("context").toClass(MySequence);
    // const conCTX = contextx.getSync<MySequence>("context");
    // console.log("ddddd", conCTX);
    this.ctx
      .bind("ws.controller")
